...
#define GLO_I0_NOM     (1.09955742875643)
#define GLO_TDP        (43200.0         )
...
static double var_uraeph(int sys, int ura)
{
    ...
    const double ura_value_glo[]={
            1, 2, 2.5, 4, 5, 7, 10, 12, 14, 16, 32, 64, 128, 256, 512, 6144.0
    };
    ...
    {
        return ura<0 || ura > 15 ? SQR(6144.0):SQR(ura_value_glo[ura]);
    }
        ...
        if (ura >=0 && ura <= 6)
            return SQR(pow(2,((double) ura/2)+1));
        else if (ura > 6 && ura < 15)
            return SQR(pow(2,(ura-2)));
        else
            return SQR(6144.0);
    }
}
...
extern double alm2clk(gtime_t time, const alm_t *alm)
{
    ...
    t=timediff(time,alm->toa);

    for (i=0;i<2;i++) {
        t-=alm->f0+alm->f1*t;
    }
    return alm->f0+alm->f1*t;
}

extern double galm2clk(gtime_t time, const galm_t *alm)
{
    double t, time_intv_sec, dif_day;
    int i;

    int yDoyAlmanac, yDoyReceiver;
    int yDoyALMArr[3];
    struct tm dateAlm;
    struct tm* dateRcv;

    glo2DateTime(alm->N_days, &yDoyALMArr[0]);
    dateAlm.tm_mday          = yDoyALMArr[0];
    dateAlm.tm_mon           = yDoyALMArr[1] > 0 ? yDoyALMArr[1] - 1 : 12;
    dateAlm.tm_year          = yDoyALMArr[2];
    mktime(&dateAlm);
    yDoyAlmanac              = dateAlm.tm_yday;
    dateRcv                  = gmtime(&time.time);
    yDoyReceiver             = dateRcv->tm_yday;
    dif_day                  = yDoyReceiver - yDoyAlmanac;
    time_intv_sec            = fmod((double)(time.time + time.sec), 3600.0 * 24.0) + (10800.0);
    t					     = (double)((dif_day) * SECONDS_IN_DAY) + (time_intv_sec - alm->t_lambda - 18.0);
...
{
    double tk,M,E,Ek,sinE,cosE,u,r,i,O,x,y,sinO,cosO,cosi,mu, omge;
    int sys, prn;
//    FILE *out_gal, *out_bds, *out_gps;
...
//    trace(4,"alm2pos : time=%s sat=%2d\n",time_str(time,3),alm->sat);
    ...
    switch ((sys=satsys(alm->sat,&prn))) {
        case SYS_GAL: mu=MU_GAL; omge=OMGE_GAL;  /*out_gal = fopen("gal_log.txt", "a")*/; break;
        case SYS_CMP: mu=MU_CMP; omge=OMGE_CMP;  /*out_bds = fopen("bds_log.txt", "a")*/; break;
        default:      mu=MU_GPS; omge=OMGE;      /*out_gps = fopen("gps_log.txt", "a")*/; break;
    }
    ...
    *dts=alm->f0+alm->f1*tk;
    ...
    switch ((sys=satsys(alm->sat,&prn))) {
        case SYS_GAL: mu=MU_GAL; omge=OMGE_GAL;  /*fprintf(out_gal, "%d %f \t %f \t %f \t %f \n", alm->sat, time.time + time.sec, rs[0], rs[1], rs[2]);*/
            /*fclose(out_gal);*/ break;
        case SYS_CMP: mu=MU_CMP; omge=OMGE_CMP;  /*fprintf(out_bds, "%d %f \t %f \t %f \t %f \n", alm->sat, time.time + time.sec, rs[0], rs[1], rs[2]);*/
        /*fclose(out_bds);*/ break;
        default:      mu=MU_GPS; omge=OMGE;      /*fprintf(out_gps, "%d %f \t %f \t %f \t %f \n", alm->sat, time.time + time.sec, rs[0], rs[1], rs[2]);*/
        /*    fclose(out_gps);*/ break;
    }
}

/* glonass almanac to satellite position and clock bias --------------------------------
* compute satellite position and clock bias with almanac (glonass)
* args   : gtime_t time     I   time (gpst)
*          galm_t *alm      I   glonass almanac
*          double *rs       O   satellite position (ecef) {x,y,z} (m)
*          double *dts      O   satellite clock bias (s)
* return : none
* notes  : see ref:
*---------------------------------------------------------------------------------------*/
extern void galm2pos(gtime_t time, const galm_t *alm, double *rs, double *dts)
{
    double tk, dif_day, i, n, a, lambda_dot, omega_dot, lambda, omega, E_P, deltaT, M, E_old, E, xo, yo, zo, T, time_intv_sec, term1, term2;
    double e1[3] = {0}, e2[3] = {0};
    int yDoyALMArr[3] = {0};
    struct tm tm_alm  = {0};
    struct tm* tm_recv = NULL;

    glo2DateTime(alm->N_days, &yDoyALMArr[0]);
    tm_alm.tm_mday         = yDoyALMArr[0];
    tm_alm.tm_mon          = yDoyALMArr[1] - 1;
    tm_alm.tm_year         = yDoyALMArr[2] - 1900;
    mktime(&tm_alm);

    tm_recv                 = gmtime(&time.time);
    dif_day                 = tm_recv->tm_yday - tm_alm.tm_yday;
    time_intv_sec           = fmod((double)((double)time.time + time.sec), 3600.0 * 24.0) + (10800.0);

    tk  = (double)((dif_day) * SECONDS_IN_DAY) + (time_intv_sec - alm->t_lambda - 18.0);
    i   = alm->i0;
    T   = GLO_TDP + alm->delta_t;
    n   = 2.0 * PI / T;
    a   = cbrt(MU_GLO/powl(n, (double)2.0));

    lambda_dot  = -10.0 * (pow(RE_GLO/a, 3.5)) * (PI/180.0) * cos(i) / SECONDS_IN_DAY;
    omega_dot   = 5.0 * (pow(RE_GLO/a, 3.5)) * (PI/180.0) * (5.0 * cos(i) * cos(i) - 1.0) / SECONDS_IN_DAY;
    lambda      = alm->lambda + (lambda_dot - OMGE_GLO) * tk;
    omega       = alm->omega + omega_dot * tk;

    term1   = tan(omega / 2.0);
    term2   = sqrt((1.0 - alm->ecc)/(1.0 + alm->ecc));
    E_P     = 2.0 * atan(term1 * term2);

    if (omega < PI)
    {
        deltaT = (E_P - alm->ecc*sin(E_P)) / n;
    }
    else
    {
        deltaT = (E_P - alm->ecc*sin(E_P)) / n + T;
    }
    ...
    while (1)
    {
        ...
        E = M + alm->ecc * sin(E_old);

        if (fabs(E - E_old) < 1E-14 || iter > MAX_ITER_KEPLER)
        {
            break;
        }
    ...
    xo = a * (cos(E) - alm->ecc);
    yo = a * (sqrt(1.0 - alm->ecc * alm->ecc) * sin(E));
    zo = 0.0;

    e1[0] = cos(omega)*cos(lambda) - sin(omega)*sin(lambda)*cos(i);
    e1[1] = cos(omega)*sin(lambda) + sin(omega)*cos(lambda)*cos(i);
    e1[2] = sin(omega)*sin(i);
    e2[0] = -sin(omega)*cos(lambda) - cos(omega)*sin(lambda)*cos(i);
    e2[1] = -sin(omega)*sin(lambda) + cos(omega)*cos(lambda)*cos(i);
    e2[2] = cos(omega)*sin(i);
    rs[0] = xo * e1[0] + yo * e2[0];
    rs[1] = xo * e1[1] + yo * e2[1];
    rs[2] = xo * e1[2] + yo * e2[2];
                     ...
                     double *var)
{
    ...
    *var= var_uraeph(SYS_GLO, geph->Ft);
}
...
/* select almanac ------------------------------------------------------------*/
static alm_t *selalm(int sat, const nav_t *nav)
{
    int i = 0;
    int constellation = 0;

    switch ((satsys(sat,NULL))) {
        case SYS_GAL: constellation = SYS_GAL; break;
        case SYS_CMP: constellation = SYS_CMP; break;
        case SYS_GLO: constellation = SYS_GLO; break;
        default:      constellation = SYS_GPS;     break;
    }

    for(;i < nav->na; ++i)
    {
        if(sat == nav->alm[i].sat)
            break;
    }
    if(i == nav->na)
        return NULL;
...
/* select glonass almanac ----------------------------------------------------*/
static galm_t *selgalm(int sat, const nav_t *nav)
{
    int i = 0;

    int constellation = SYS_GLO;

    for(;i < nav->nga; ++i)
    {
        if(sat == nav->galm[i].sat)
            break;
    }
    if(i == nav->nga)
        return NULL;
...
static int almclk(gtime_t time, gtime_t talm, int sat, const nav_t *nav,
                  double *dts)
{
    alm_t  *alm;
    galm_t *galm;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP){
        if (!(alm=selalm(sat,nav))) return 0;
        *dts=alm2clk(time,alm);
    }
    else if (sys==SYS_GLO) {
        if (!(galm=selgalm(sat,nav))) return 0;
        *dts=galm2clk(time,galm);
    }
    else if (sys==SYS_SBS) {
        //TODO:salm2clk
    }
...
/* satellite position and clock by broadcast almanac -----------------------*/
static int almpos(gtime_t time, int sat, const nav_t *nav,
                  double *rs, double *dts, int *svh)
{
    alm_t  *alm;
    galm_t *galm;
//    salm_t *salm;
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        ...
        alm2pos(time,alm,rst,dtst);
        *svh=alm->svh;
    }
    else if (sys==SYS_GLO) {
        if (!(galm=selgalm(sat,nav))) return 0;

        galm2pos(time, galm, rs, dts);
        time=timeadd(time,tt);
        galm2pos(time, galm, rst, dtst);
                  ...
                  int *svh)
{
    ...
    *svh=0;
    ...
    switch (ephopt) {
        ...
        case ALMOPT_MGA   : return almpos     (time,sat,nav,rs,dts,svh);
    }
    *svh=-1;
    return 0;
}
...
*-----------------------------------------------------------------------------*/
extern void satposs(gtime_t tobs, const obsd_t *obs, int n, const nav_t *nav,
                    int ephopt, double *rs, double *dts, double *var, int *svh, RTK_Nav_Type navtype_t)
{
    gtime_t time[2*MAXOBS]={{0}};
    double dt,pr;
    int i,j;

    trace(3,"satposs : teph=%s n=%d ephopt=%d\n",time_str(tobs,3),n,ephopt);
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if(navtype_t == NAV_TYPE_EPHEMERIS)
        {
            /* satellite clock bias by broadcast ephemeris */
            if (!ephclk(time[i],tobs,obs[i].sat,nav,&dt)) {
                trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
                continue;
            }
        }
        else if (navtype_t == NAV_TYPE_ALMANAC)
        {
            /* satellite clock bias by almanac */
            if (!almclk(time[i],tobs,obs[i].sat,nav,&dt)) {
                trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
                continue;
            }
        }
        ...
        if (!satpos(time[i],tobs,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,svh+i)) {
        ...
        if(navtype_t == NAV_TYPE_EPHEMERIS)
        {
            ...
            if (dts[i*2]==0.0) {
                if (!ephclk(time[i],tobs,obs[i].sat,nav,dts+i*2)) continue;
                dts[1+i*2]=0.0;
                *var=SQR(STD_BRDCCLK);
            }
        }
        else if (navtype_t == NAV_TYPE_ALMANAC)
        {
            /* if no precise clock available, use broadcast clock instead */
            if (dts[i*2]==0.0) {
                if (!almclk(time[i],tobs,obs[i].sat,nav,dts+i*2)) continue;
                dts[1+i*2]=0.0;
                *var=SQR(STD_BRDCCLK);
            }
        }
