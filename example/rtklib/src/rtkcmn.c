...
const double chisqr_2[100]={      /* chi-sqr(n) (alpha=0.005) */
...
const double chisqr_3[100]={      /* chi-sqr(n) (alpha=0.01) */
...
const double chisqr_4[100]={      /* chi-sqr(n) (alpha=0.1) */
...
const prcopt_t prcopt_default={     /* defaults processing options */
    PMODE_SINGLE,0,2,SYS_NONE,      /* mode,soltype,nf,navsys */
...
extern gtime_t epoch2time(const double *ep)
{
    ...
    sec=(int)floor(ep[5]);
    time.time=(time_t)days*86400+(int)ep[3]*3600+(int)ep[4]*60+sec; // Time in seconds
    time.sec=ep[5]-sec; // Fraction of second under 1 second
    return time;
}
...
void glo2DateTime(double e_days, int dateArr[])
{
    int32_t curYear, curMonth, curDay, tb_mins;
    ...
    int yearr = aTime->tm_year + 1900; // Year is # years since 1900

    uint8_t N4_YEARS_CURRENT = ((yearr - 1996) / 4) + 1;
    double JD0 = 1461*( N4_YEARS_CURRENT - 1) + (e_days) + 2450082.5;
    double JDN = floor(JD0 + 0.5);

    int64_t a  = JDN + 32044;
    int64_t b  = floor((4*a + 3)/146097);
    int64_t c  = a - floor(((146097 * b)/4));
    int64_t d  = floor((4 * c + 3)/1461);
    int64_t e  = c - floor((1461 * d)/4);
    int64_t m  = floor((5*e + 2)/153);

    curDay   = floor(e - ((153*m+2)/5) + 1);
    curMonth = (m + 3 - 12*floor((m/10)));
    curYear  = 100 * b + d - 4800 + floor((m/10));
    tb_mins = (float )500.0;
    int32_t tb_hours = (tb_mins / 60) - 3;
    ...
    dateArr[0] = (int)curDay;
    dateArr[1] = (int)curMonth;
    dateArr[2] = (int)curYear;
}
...
extern void time2str(gtime_t t, char *s, int n)
{
    ...
    time2epoch(t,ep);
    (void)sprintf(s,"%04.0f/%02.0f/%02.0f %02.0f:%02.0f:%0*.*f",ep[0],ep[1],ep[2],
...
extern int adjgalweek(int week)
{
    ...
    (void)time2gpst(utc2gpst(timeget()),&w);
    if (w<1560) w=1560; /* use 2009/12/1 if time is earlier than 2009/12/1 */
    int adjweek = week+(w-week+1)/1024*1024;
    adjweek -= 1024;
    //return adjweek;
    return w;
}
/*-----------------------------------------------------------------------------*/
/* adjust BeiDou week number ------------------------------------------------------
* adjust gps week number using cpu time
* args   : int   week       I   not-adjusted gps week number (0-1023)
* return : adjusted gps week number
*-----------------------------------------------------------------------------*/
...
/* compare almanac
 ---------------------------------------------------------*/
static int cmpalm(const void *p1, const void *p2)
{
    alm_t *q1=(alm_t *)p1,*q2=(alm_t *)p2;
    return q1->toa.time!=q2->toa.time?(int)(q1->toa.time-q2->toa.time): q1->sat-q2->sat;
}
/* compare glonass almanac
 ---------------------------------------------------------*/
static int cmpgalm(const void *p1, const void *p2)
{
    galm_t *q1=(galm_t *)p1,*q2=(galm_t *)p2;
    return q1->t_lambda!=q2->t_lambda?(int)(q1->t_lambda-q2->t_lambda): q1->sat-q2->sat;
}
...
/* sort and unique almanac
 -------------------------------------------------*/
static void uniqalm(nav_t *nav)
{
    alm_t *nav_alm;
    int i,j;
    ...
    qsort(nav->alm,nav->na,sizeof(alm_t),cmpalm);

    for (i=1,j=0;i<nav->na;i++) {
        if (nav->alm[i].sat!=nav->alm[j].sat) {
            nav->alm[++j]=nav->alm[i];
        }
    }
    nav->na=j+1;

    if (!(nav_alm=(alm_t *)realloc(nav->alm,sizeof(alm_t)*nav->na))) {
        trace(1,"uniqalm malloc error n=%d\n",nav->na);
        free(nav->alm); nav->alm=NULL; nav->na=nav->namax=0;
        return;
    }
    nav->alm=nav_alm;
    nav->namax=nav->na;
...
/* sort and unique glonass almanac
 -------------------------------------------------*/
static void uniqgalm(nav_t *nav)
{
    galm_t *nav_galm;
    int i,j;

    trace(3,"uniqgalm: n=%d\n",nav->na);

    if (nav->na<=0) return;

    qsort(nav->galm,nav->na,sizeof(galm_t),cmpgalm);

    for (i=1,j=0;i<nav->na;i++) {
        if (nav->galm[i].sat!=nav->galm[j].sat) {
            nav->galm[++j]=nav->galm[i];
        }
    }
    nav->na=j+1;

    if (!(nav_galm=(galm_t *)realloc(nav->galm,sizeof(galm_t)*nav->na))) {
        trace(1,"uniqgalm malloc error n=%d\n",nav->na);
        free(nav->galm); nav->galm=NULL; nav->na=nav->namax=0;
        return;
    }
    nav->galm=nav_galm;
    nav->namax=nav->na;
...
/* unique almanac --------------------------------------------------------------
* unique almanac in navigation data and update carrier wave length
...
extern double model_grav(int sys, double* rr, double* rs)
{
    ...
    double rec_module = norm(rr,3);
    double sat_module = norm(rs,3);
    double distance   = norm(diff,3);
    ...
    double d_grav = 2 * MU / (CLIGHT * CLIGHT) * log((rec_module + sat_module + distance)/(rec_module+sat_module-distance));
...
{
    int    code = ubx_sig(sys, sigid); //! ubx kodunu rtklib koduna mapler.
...
extern double dopp_var(double speed, double snr, double lam)
{
    ...
    double gps_lam = CLIGHT / FREQ1;
    double rate = pow(lam / gps_lam, 2);

    static const double look_up[10][7] =
            {0.53,0.43,0.28,0.21,0.13,0.09,0.07,
             2.04,1.61,1.08,0.71,0.55,0.32,0.24,
             2.37,2.00,1.43,0.86,0.53,0.29,0.18,
             3.08,2.33,1.60,1.06,0.99,0.28,0.22,
             2.54,1.87,1.25,0.83,0.54,0.27,0.17,
             1.26,1.36,1.48,0.80,0.53,0.27,0.15,
             1.37,1.29,0.87,0.55,0.44,0.25,0.16,
             1.44,1.25,1.09,0.73,0.37,0.26,0.16,
             1.33,0.9,0.58,0.47,0.31,0.26,0.11,
             1.01,0.92,0.61,0.53,0.31,0.31,0.14};
    ...
    if(snr > 30)
    {
        ...
        if (j > 6)
            ...
            return (look_up[i][j] * look_up[i][j] * rate);
    }
    else
    {
        return ((look_up[i][0] + 1) * (look_up[i][0] + 1) * rate);
    }
}
